...
describe("aws skill", () => {
  it("keeps AWS access split by text-guided read and write permissions", async () => {
    const raw = await fs.readFile(new URL("../skills/aws/SKILL.md", import.meta.url), "utf8");

    expect(raw).toContain("aws.read");
    expect(raw).toContain("aws.write");
    expect(raw).not.toContain("aws:aws.read");
    expect(raw).not.toContain("aws:aws.write");
  ...
  it("keeps credentials in the runtime secret env without profile or skill-local env requirements", async () => {
    ...
    expect(raw).toContain("AWS_SESSION_TOKEN");
    ...
    expect(raw).toContain("do not require `AWS_PROFILE`");
    expect(raw).not.toContain("AWS_JALA_");
  });
  ...
  it("checks for aws CLI and defers to install-tool when missing", async () => {
    const raw = await fs.readFile(new URL("../skills/aws/SKILL.md", import.meta.url), "utf8");

    expect(raw).toContain("install-tool");
    expect(raw).toContain("If the `aws` CLI binary is missing, tell the user to use `install-tool` first");
  });

  it("keeps growing workflows as reference-backed use case recipes", async () => {
    const raw = await fs.readFile(new URL("../skills/aws/SKILL.md", import.meta.url), "utf8");
    const monthlyReport = await fs.readFile(
      new URL("../skills/aws/references/use-cases/cost-summary-monthly-report.md", import.meta.url),
      "utf8",
    );
    const ec2Inventory = await fs.readFile(
      new URL("../skills/aws/references/use-cases/ec2-inventory.md", import.meta.url),
      "utf8",
    );
    const iamReview = await fs.readFile(
      new URL("../skills/aws/references/use-cases/iam-access-review.md", import.meta.url),
      "utf8",
    );
    ...
    expect(monthlyReport).toContain("workspace/reports/aws/cost-summary");
    expect(monthlyReport).not.toContain("aws-jala");

    expect(ec2Inventory).toContain("# EC2 Inventory");
    expect(ec2Inventory).toContain("Required permission: `aws.read`");

    expect(iamReview).toContain("# IAM Access Review");
    expect(iamReview).toContain("Required permission: `aws.read`");
  });

  it("provides the platform-context reference", async () => {
    const raw = await fs.readFile(new URL("../skills/aws/SKILL.md", import.meta.url), "utf8");
    const platformContext = await fs.readFile(
      new URL("../skills/aws/references/platform-context.md", import.meta.url),
      "utf8",
    );

    expect(raw).toContain("references/platform-context.md");
    expect(platformContext).toContain("# AWS Platform Context");
    expect(platformContext).not.toContain("Jala");
  });
});
