...
// Get team ID (auto-detect if not set)
export async function getTeamId() {
  ...
  const team = teams[0];
  console.error(`Detected team: ${team.name} (${team.id})`);
  console.error('Set CLICKUP_TEAM_ID to skip detection.\n');

  return team.id;
}

// Get current user ID (auto-detect if not set)
export async function getUserId() {
  ...
  console.error(`Detected user: ${user.username} (${userId})`);
  console.error('Set CLICKUP_USER_ID to skip detection.\n');
