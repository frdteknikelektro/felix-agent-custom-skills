...
`jala-web` follows Git Flow with project-specific branch policy for production, next-release, staging, and feature work.

Canonical repository: `https://gitlab.com/atnic/jala-web`
...
- Do not update `master`, `develop`, or `feature/compile-to-test` directly.
...
- Normal development PRs must not target `develop` or `master`.
